...
Public Class Exec_EWrapperImpl

    Implements EWrapper
    '! [ewrapperimpl]
    '! [socket_declare]

    Public Exec_eReaderSignal As EReaderSignal
    Public Exec_socketClient As EClientSocket
    '! [socket_declare]
    Public Exec_nextOrderId As Integer

    'Public Event Exec_on_current_time(Ewrapper_impl_time As EWrapperImpl, eventArgs As Exec_on_current_time_event)
    Public Shared Event Exec_on_current_time(ByVal time As Long)
    Public Shared Event Exec_on_price_return(ByVal tickerId, ByVal field, ByVal price, ByVal canAutoExecute)
    Public Shared Event Exec_on_size_return(ByVal tickerId, ByVal field, ByVal size)
    Public Shared Event Exec_on_string_return(ByVal tickerId, ByVal field, ByVal string_return)
    Public Shared Event Exec_on_open_ords(ByVal orderId As Integer, ByVal contract As IBApi.Contract, ByVal order As IBApi.Order, ByVal orderState As IBApi.OrderState)
    Public Shared Event Exec_on_open_ords_end()
    Public Shared Event Exec_on_ords_status(ByVal orderId As Integer, ByVal status As String, ByVal filled As Double, ByVal remaining As Double, ByVal avgFillPrice As Double,
                                       ByVal permId As Integer, ByVal parentId As Integer, ByVal lastFillPrice As Double, ByVal clientId As Integer, ByVal whyHeld As String)
    Public Shared Event Exec_on_exec_details(ByVal reqId As Integer, ByVal contract As IBApi.Contract, ByVal execution As IBApi.Execution)
    Public Shared Event Exec_on_cmssn_rpt(ByVal commissionReport As IBApi.CommissionReport)
    Public Shared Event Exec_on_next_valid_id_return(ByVal next_order_ID As Integer)

    '! [socket_init]
    Sub New()
        Exec_eReaderSignal = New EReaderMonitorSignal
        Exec_socketClient = New EClientSocket(Me, Exec_eReaderSignal)
    End Sub
    '! [socket_init]

    Function serverVersion() As Integer
        serverVersion = Exec_socketClient.ServerVersion
    End Function
    ...
    Public Sub commissionReport(commissionReport As IBApi.CommissionReport) Implements IBApi.EWrapper.commissionReport
        ...
        RaiseEvent Exec_on_cmssn_rpt(commissionReport)
    ...
    Public Sub connectAck() Implements IBApi.EWrapper.connectAck
        ...
        If Exec_socketClient.AsyncEConnect Then
            Exec_socketClient.startApi()
        End If
    End Sub
    '! [connectack]
    ...
    Public Sub currentTime(time As Long) Implements IBApi.EWrapper.currentTime
        ...
        'RaiseEvent Exec_on_current_time(Me, New Exec_on_current_time_event With {.time = time})
        RaiseEvent Exec_on_current_time(time)
    ...
    Public Sub [error](id As Integer, errorCode As Integer, errorMsg As String) Implements IBApi.EWrapper.error
        ...
        If errorCode = 2106 Or errorCode = 2104 Then
        Else
            MsgBox("Error - Id [" & id & "] ErrorCode [" & errorCode & "] ErrorMsg [" & errorMsg & "]")
        End If
    ...
    Public Sub execDetails(reqId As Integer, contract As IBApi.Contract, execution As IBApi.Execution) Implements IBApi.EWrapper.execDetails
        ...
        RaiseEvent Exec_on_exec_details(reqId, contract, execution)
    ...
    Public Sub nextValidId(orderId As Integer) Implements IBApi.EWrapper.nextValidId
        ...
        Exec_nextOrderId = orderId

        RaiseEvent Exec_on_next_valid_id_return(Exec_nextOrderId)
        MsgBox("Exec_order_ID: " & Exec_nextOrderId)
    ...
    Public Sub openOrder(orderId As Integer, contract As IBApi.Contract, order As IBApi.Order, orderState As IBApi.OrderState) Implements IBApi.EWrapper.openOrder
        ...
        RaiseEvent Exec_on_open_ords(orderId, contract, order, orderState)
    ...
    Public Sub openOrderEnd() Implements IBApi.EWrapper.openOrderEnd
        ...
        RaiseEvent Exec_on_open_ords_end()
    ...
    Public Sub orderStatus(orderId As Integer, status As String, filled As Double, remaining As Double, avgFillPrice As Double, permId As Integer, parentId As Integer, lastFillPrice As Double, clientId As Integer, whyHeld As String) Implements IBApi.EWrapper.orderStatus
        ...
        RaiseEvent Exec_on_ords_status(orderId, status, filled, remaining, avgFillPrice, permId, parentId, lastFillPrice, clientId, whyHeld)
    ...
    Public Sub tickPrice(tickerId As Integer, field As Integer, price As Double, canAutoExecute As Integer) Implements IBApi.EWrapper.tickPrice
        ...
        RaiseEvent Exec_on_price_return(tickerId, field, price, canAutoExecute)
    ...
    Public Sub tickSize(tickerId As Integer, field As Integer, size As Integer) Implements IBApi.EWrapper.tickSize
        ...
        RaiseEvent Exec_on_size_return(tickerId, field, size)
    ...
    '! [tickstring]
    Public Sub tickString(tickerId As Integer, field As Integer, string_return As String) Implements IBApi.EWrapper.tickString
        'Console.WriteLine("Tick string. Ticker Id:" & CStr(tickerId) & ", Type: " & TickType.getField(field) & ", Value: " & value)

        RaiseEvent Exec_on_string_return(tickerId, field, string_return)
